  ...
  useEffect(() => {
    async function fetchDates() {
      ...
      const { data } = await supabase
        .from('rejections')
        .select('rejection_date')
        .in('application_id', ids);
      setDates(data.map((d) => d.rejection_date));
    }
    fetchDates();
  }, []);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Rejections</h1>
      ...
      <EntriesBarChart dates={filteredDates} rangeMs={rangeMs} bucket={bucket} label="Rejections over time" color="rgba(244, 63, 94, 0.6)" />
      <RejectionsTable />
    </div>
  );
}
