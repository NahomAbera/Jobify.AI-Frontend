...
export default function Interviews() {
  ...
  useEffect(() => {
    async function fetchDates() {
      ...
      setDates(data.map((d) => d.invitation_date));
    }
    fetchDates();
  }, []);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Select value={rangeMs} onChange={(e) => setRangeMs(Number(e.target.value))} className="w-44">
          {ranges.map((r) => (
            <option key={r.ms} value={r.ms}>Past {r.label}</option>
          ))}
        </Select>
      ...
      <EntriesBarChart dates={filteredDates} rangeMs={rangeMs} bucket={bucket} label="Interviews over time" color="rgba(56, 189, 248, 0.6)" />
      <InterviewsTable />
    </div>
  );
}
